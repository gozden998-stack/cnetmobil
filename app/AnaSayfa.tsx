...
export default function AnaSayfa({ selectedBranch, setAppMode, config, gidisatData = [], personelData = [], hedeflerData = [], izinlerData = [] }: any) {
    ...
    // --- YENİ TASARIM KARTLAR İÇİN DİNAMİK VERİ HESAPLAMALARI ---
    ...
    return (
        <div className="min-h-screen bg-[#F8F9FB] p-4 md:p-8 font-sans text-slate-800 animate-in fade-in duration-500 overflow-x-hidden">
            ...
            <div className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-100 mb-8 flex flex-col xl:flex-row gap-6 justify-between">
                ...
                {/* SAĞ KISIM: "AYIN EN İYİ MAĞAZASI" PODYUMU */}
            ...
            {/* 2. BÖLÜM: YENİ 4'LÜ GRİD (PERSONEL, MAĞAZA, VİZYON & MOTİVASYON) */}
            <div className={`grid grid-cols-1 md:grid-cols-2 ${!isBlocked ? 'lg:grid-cols-4' : 'lg:grid-cols-1'} gap-5 mb-8`}>
                ...
                {!isBlocked && (
                    <div className="relative overflow-hidden bg-[#0A1128] rounded-[2rem] p-6 shadow-xl flex flex-col justify-between min-h-[220px] border border-blue-900/40">
                        ...
                        <div className="grid grid-cols-3 gap-2 mt-4 pt-4 border-t border-blue-900/40">
                            <button onClick={() => setActiveDrawer('personel')} className="bg-[#131F42] hover:bg-[#1c2e63] text-slate-300 py-2.5 rounded-xl text-[10px] font-bold uppercase transition-colors tracking-wider border border-blue-800/30">SIRALAMALAR</button>
                            <button onClick={() => setActiveModal('hedefler')} className="bg-[#053B29] hover:bg-[#075239] text-emerald-300 py-2.5 rounded-xl text-[10px] font-bold uppercase transition-colors tracking-wider border border-emerald-600/30">PUANLAR</button>
                            <button onClick={() => setActiveDrawer('personel')} className="bg-[#1D4ED8] hover:bg-blue-600 text-white py-2.5 rounded-xl text-[10px] font-black uppercase transition-colors tracking-wider shadow-lg shadow-blue-600/30">İNCELE</button>
                        </div>
                    </div>
                )}
                ...
                {!isBlocked && (
                    <div className="relative overflow-hidden bg-gradient-to-br from-[#2D124D] to-[#1C0A35] rounded-[2rem] p-6 shadow-xl flex flex-col justify-between min-h-[220px] border border-purple-800/40">
                        ...
                        <div className="grid grid-cols-3 gap-2 mt-2 pt-3 border-t border-purple-800/40 clear-both">
                            <button onClick={() => setActiveModal('hedefler')} className="bg-[#3D1A6D] hover:bg-[#4d2287] text-purple-200 py-2.5 rounded-xl text-[10px] font-bold uppercase transition-colors tracking-wider border border-purple-600/30">HEDEFLER</button>
                            <button onClick={() => setActiveDrawer('magaza')} className="bg-[#3D1A6D] hover:bg-[#4d2287] text-purple-200 py-2.5 rounded-xl text-[10px] font-bold uppercase transition-colors tracking-wider border border-purple-600/30">KALAN ADET</button>
                            <button onClick={() => setActiveDrawer('magaza')} className="bg-[#8B5CF6] hover:bg-purple-500 text-white py-2.5 rounded-xl text-[10px] font-black uppercase transition-colors tracking-wider shadow-lg shadow-purple-600/30">DETAYLAR</button>
                        </div>
                    </div>
                )}
                    ...
                    <div className="relative overflow-hidden bg-gradient-to-br from-[#0C6967] to-[#064e4c] rounded-[2rem] p-6 shadow-xl flex flex-col justify-between min-h-[220px] text-white">
                        ...
                        <div className="flex justify-between items-start relative z-10">
                            <div>
                                ...
                                <h3 className="text-base font-black leading-snug mt-2 max-w-[180px]">"Müşteri geri çevirmek yok, mağazada yok yok!"</h3>
                                <p className="text-[11px] text-emerald-100/90 mt-2 font-medium">Bu vizyonla bu ay hedefi patlatıyoruz! 🚀</p>
                            </div>
                        ...
                        {/* Ekip/Başarı İllüstrasyon Temsili */}
                        <div className="relative z-10 flex justify-center items-end mt-4 pt-2 -mb-2 opacity-90">
                            <div className="flex items-center gap-1 text-2xl animate-pulse">
