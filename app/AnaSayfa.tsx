...
export default function AnaSayfa({ selectedBranch, setAppMode, config, gidisatData = [], personelData = [], hedeflerData = [], izinlerData = [] }: any) {
    ...
    return (
        <div className="min-h-screen bg-[#F8F9FB] p-4 md:p-8 font-sans text-slate-800 overflow-x-hidden space-y-8 animate-in fade-in duration-500 selection:bg-blue-600 selection:text-white">
            ...
            {/* ========================================================= */}
            {/* 2. KATMAN: 4'LÜ PASTEL PREMİUM KART SERİSİ (image_26431e.jpg Birebir Clone) */}
            {/* ========================================================= */}
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">

                {/* 1. KART: PERSONEL METRİKLERİ (Pastel Bebek Mavisi) */}
                <div className="bg-white rounded-[2.5rem] p-7 border border-blue-100 shadow-sm hover:shadow-md hover:border-blue-300 transition-all duration-300 flex flex-col justify-between group">
                    <div>
                        <div className="flex items-center gap-2 text-blue-600 mb-2">
                            <div className="w-7 h-7 rounded-xl bg-blue-50 flex items-center justify-center font-bold text-xs">👥</div>
                            <span className="text-[11px] font-black tracking-widest uppercase">PERSONEL METRİKLERİ</span>
                        </div>
                        <h3 className="text-xl font-extrabold text-slate-800 tracking-tight">Güçlü Ekip, Büyük Başarı!</h3>

                        {/* 3'lü Ekip İllüstrasyonu (CSS ile Vektör Simülasyonu) */}
                        <div className="my-6 flex justify-center items-center -space-x-3 py-1">
                            <div className="w-14 h-14 rounded-full border-4 border-white bg-blue-100 flex items-center justify-center text-xl shadow-md transform -rotate-12">👨‍💻</div>
                            <div className="w-16 h-16 rounded-full border-4 border-white bg-blue-600 text-white flex items-center justify-center text-2xl shadow-lg z-10 -translate-y-1 font-bold">👩‍💼</div>
                            <div className="w-14 h-14 rounded-full border-4 border-white bg-sky-100 flex items-center justify-center text-xl shadow-md transform rotate-12">🧑‍💻</div>
                        </div>

                        {/* Alt İstatistikler */}
                        <div className="grid grid-cols-3 gap-1 pt-3 border-t border-slate-100 text-center">
                            <div>
                                <div className="text-2xl font-black text-slate-800">{tumSirketPersonelleri.length || 24}</div>
                                <div className="text-[10px] font-bold text-slate-400 mt-0.5">Toplam Pers.</div>
                            </div>
                            <div>
                                <div className="text-2xl font-black text-blue-600">{aktifPersoneller.length || 21}</div>
                                <div className="text-[10px] font-bold text-slate-400 mt-0.5">Aktif Pers.</div>
                            </div>
                            <div>
                                <div className="text-2xl font-black text-emerald-500">{ortalamaPerformans}</div>
                                <div className="text-[10px] font-bold text-slate-400 mt-0.5">Ort. Performans</div>
                            </div>
                        </div>
                    </div>

                    <button onClick={() => setActiveDrawer('personel')} className="w-full mt-6 py-3 bg-blue-50/70 hover:bg-blue-600 hover:text-white text-blue-600 font-extrabold rounded-2xl text-xs transition-all flex items-center justify-center gap-2 cursor-pointer group/btn">
                        <span>Detayları İncele</span>
                        <span className="group-hover/btn:translate-x-1 transition-transform">→</span>
                    </button>
                </div>

                {/* 2. KART: MAĞAZA PERFORMANSI (Pastel Lila & Canlı Donut Grafiği) */}
                <div className="bg-white rounded-[2.5rem] p-7 border border-purple-100 shadow-sm hover:shadow-md hover:border-purple-300 transition-all duration-300 flex flex-col justify-between group">
                    <div>
                        <div className="flex items-center gap-2 text-purple-600 mb-2">
                            <div className="w-7 h-7 rounded-xl bg-purple-50 flex items-center justify-center font-bold text-xs">📊</div>
                            <span className="text-[11px] font-black tracking-widest uppercase">MAĞAZA PERFORMANSI</span>
                        </div>
                        <h3 className="text-xl font-extrabold text-slate-800 tracking-tight">Hedefe Doğru İlerliyoruz!</h3>

                        {/* Donut Chart & Yan Değerler */}
                        <div className="my-4 flex items-center justify-between px-1">
                            <div className="text-left">
                                <span className="text-2xl font-black text-slate-800 block">{anaHedef}</span>
                                <span className="text-[10px] font-bold text-slate-400">Aylık Hedef</span>
                            </div>

                            {/* Gerçek SVG Donut Grafiği */}
                            <div className="relative w-28 h-28 flex items-center justify-center shrink-0">
                                <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                                    <circle cx="50" cy="50" r="38" stroke="#F1F5F9" strokeWidth="11" fill="transparent" />
                                    <circle 
                                        cx="50" cy="50" r="38" 
                                        stroke="url(#purple_violet)" 
                                        strokeWidth="11" 
                                        strokeDasharray={238.76} 
                                        strokeDashoffset={238.76 - ((tamamlananYuzde / 100) * 238.76)} 
                                        strokeLinecap="round" 
                                        fill="transparent" 
                                        className="transition-all duration-1000 ease-out"
                                    />
                                    <defs>
                                        <linearGradient id="purple_violet" x1="0%" y1="0%" x2="100%" y2="100%">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                                    <span className="text-2xl font-black text-slate-800 -mb-1">%{tamamlananYuzde}</span>
                                    <span className="text-[8px] font-bold text-slate-400 uppercase tracking-wider">Tamamlandı</span>
                                </div>
                            </div>

                            <div className="text-right">
                                <span className="text-2xl font-black text-emerald-500 block">{anaSatis}</span>
                                <span className="text-[10px] font-bold text-slate-400">Gerçekleşen</span>
                            </div>
                        </div>

                        {/* Kalan Adet & Roket */}
                        <div className="bg-purple-50/60 border border-purple-100 rounded-2xl p-3 flex items-center justify-between mt-1">
                            <div>
                                <span className="text-xs font-black text-purple-950">{kalanHedef} Adet Kaldı</span>
                                <span className="text-[10px] font-medium text-slate-500 block">Projeksiyon: {anaProjeksiyon}</span>
                            </div>
                            <span className="text-xl animate-bounce">🚀</span>
                        </div>
                    </div>

                    <button onClick={() => setActiveModal('hedefler')} className="w-full mt-6 py-3 bg-purple-50/70 hover:bg-purple-600 hover:text-white text-purple-600 font-extrabold rounded-2xl text-xs transition-all flex items-center justify-center gap-2 cursor-pointer group/btn">
                        <span>Performansı Gör</span>
                        <span className="group-hover/btn:translate-x-1 transition-transform">→</span>
                    </button>
                </div>

                {/* 3. KART: MAĞAZA VİZYONU (Pastel Nane Yeşili & Kesişen Halkalar) */}
                <div className="bg-white rounded-[2.5rem] p-7 border border-emerald-100 shadow-sm hover:shadow-md hover:border-emerald-300 transition-all duration-300 flex flex-col justify-between group">
                    <div>
                        <div className="flex items-center gap-2 text-emerald-600 mb-2">
                            <div className="w-7 h-7 rounded-xl bg-emerald-50 flex items-center justify-center font-bold text-xs">🎯</div>
                            <span className="text-[11px] font-black tracking-widest uppercase">MAĞAZA VİZYONU</span>
                        </div>
                        <h3 className="text-xl font-extrabold text-slate-800 tracking-tight">Birlikte Daha Güçlüyüz!</h3>

                        {/* Kesişen 3 Yeşil Halka İllüstrasyonu */}
                        <div className="my-6 py-1 flex flex-col items-center">
                            <div className="flex items-center justify-center -space-x-5">
                                <div className="w-14 h-14 rounded-full border-4 border-white bg-emerald-100 flex items-center justify-center text-emerald-700 font-black text-xs shadow-sm">🤝</div>
                                <div className="w-16 h-16 rounded-full border-4 border-white bg-emerald-500 text-white flex items-center justify-center font-black text-base shadow-md z-10 -translate-y-1">📈</div>
                                <div className="w-14 h-14 rounded-full border-4 border-white bg-teal-100 flex items-center justify-center text-teal-700 font-black text-xs shadow-sm">⭐</div>
                            </div>
                            <div className="flex justify-between w-full text-[9px] font-black text-slate-400 uppercase tracking-tighter mt-3 px-1">
                                <span>Müşteri Mem.</span>
                                <span>Satış Perf.</span>
                                <span>Ekip Ruhu</span>
                            </div>
                        </div>
                        ...
                        <div className="bg-emerald-50/70 border border-emerald-100 rounded-2xl p-3 text-center">
                            <p className="text-xs font-black text-emerald-900 italic">"Müşteri geri çevirmek yok, mağazada yok yok!"</p>
                        </div>
                    </div>

                    <button onClick={() => setActiveModal('departman')} className="w-full mt-6 py-3 bg-emerald-50/70 hover:bg-emerald-600 hover:text-white text-emerald-600 font-extrabold rounded-2xl text-xs transition-all flex items-center justify-center gap-2 cursor-pointer group/btn">
                        <span>Vizyonumuzu Keşfet</span>
                        <span className="group-hover/btn:translate-x-1 transition-transform">→</span>
                    </button>
                </div>

                {/* 4. KART: MOTİVASYON KÖŞESİ (Pastel Krem/Sarı & Dağ Zirvesi) */}
                <div className="bg-gradient-to-b from-amber-50/40 via-orange-50/20 to-amber-100/50 rounded-[2.5rem] p-7 border border-amber-200/60 shadow-sm hover:shadow-md hover:border-amber-300 transition-all duration-300 flex flex-col justify-between group relative overflow-hidden">
                    
                    {/* Arka Plan Dağ Silüeti SVG */}
                    <div className="absolute inset-x-0 bottom-0 h-28 pointer-events-none opacity-40 overflow-hidden">
                        <svg viewBox="0 0 200 100" className="w-full h-full preserve-3d" fill="none">
                            <path d="M0 100L50 45L90 75L150 25L200 65V100H0Z" fill="#FDE68A" />
                            <path d="M40 100L90 60L130 85L180 40L200 55V100H40Z" fill="#F59E0B" opacity="0.25" />
                        </svg>
                    </div>

                    <div className="relative z-10">
                        <div className="flex items-center gap-2 text-amber-600 mb-1">
                            <div className="w-7 h-7 rounded-xl bg-amber-100/80 flex items-center justify-center font-bold text-xs">🏆</div>
                            <span className="text-[11px] font-black tracking-widest uppercase">MOTİVASYON KÖŞESİ</span>
                        </div>
                        
                        <div className="text-5xl text-amber-400 font-serif font-black leading-none -mb-2 mt-2">“</div>
                        <h3 className="text-xl font-black text-slate-800 leading-snug">Bugün attığın adım, yarın liderliğini getirir!</h3>
                        <p className="text-xs font-bold text-slate-500 mt-2">Odaklan • Hedefe Ulaş • Kazan!</p>
                    </div>

                    {/* Zirve Bayrağı Butonu */}
                    <div className="relative z-10 mt-6 pt-4 border-t border-amber-200/50 flex items-center justify-between">
                        <div>
                            <span className="text-[10px] font-bold text-amber-800 uppercase block tracking-wider">Aylık Genel Puan</span>
                            <span className="text-xl font-black text-amber-950">{magazaTahminPuan.toFixed(1)} Pts</span>
                        </div>
                        <button onClick={() => setActiveDrawer('personel')} className="px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-black rounded-xl text-xs shadow-sm transition-all group-hover:scale-105 cursor-pointer">
                            İlham Al →
                        </button>
                    </div>
                </div>
