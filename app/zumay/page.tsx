...
const ZUMAY_PASS = "ZUMAY123"; // Buraya Zumay için belirlediğin şifreyi yazabilirsin
...
export default function ZumayPortal() {
  ...
  // VERİYİ GÜVENLİ SUNUCUDAN (BACKEND) ÇEKİYORUZ
  const loadData = async () => {
    try {
      const res = await fetch('/api/zumay'); // Google'a değil, kendi güvenli sunucumuza gidiyoruz
      const data = await res.json();
      
      if (data.brands?.values) setBrandDb(data.brands.values.map((row: any) => ({ name: row[0], logo: row[1] })));
      if (data.disKanal?.values) setDisKanalData(data.disKanal.values);
      if (data.devices?.values) {
        setDb(data.devices.values.map((row: any) => ({
      ...
      if (data.config?.values) {
        const m: any = {};
        data.config.values.forEach((row: any) => { m[row[0]] = isNaN(Number(row[1])) ? row[1] : parseFloat(row[1]); });
        setConfig(m);
      }
      setLoading(false);
    } catch (e) { setLoading(false); alert("Veri yüklenirken hata oluştu."); }
  };
  ...
  // VERİYİ GÜVENLİ SUNUCU ÜZERİNDEN KAYDEDİYORUZ
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-[#111111] text-white font-black tracking-widest">YÜKLENİYOR...</div>;
    ...
    <div className="flex flex-col md:flex-row min-h-screen font-sans bg-[#111111] text-white">
      <nav className="w-full md:w-[280px] bg-[#0a0a0f] border-r border-white/5 flex flex-col shrink-0">
        ...
        <div className="flex-1 px-4 mt-8 space-y-2">
            <button onClick={() => {setAppMode('ana_sayfa'); resetAll();}} className={`w-full flex items-center px-4 py-4 rounded-xl text-xs font-bold uppercase ${appMode === 'ana_sayfa' ? 'bg-red-600/10 text-red-500 border-l-2 border-red-500' : 'text-slate-400 hover:bg-white/5'}`}>Ana Sayfa</button>
            <button onClick={() => {setAppMode('alim'); resetAll();}} className={`w-full flex items-center px-4 py-4 rounded-xl text-xs font-bold uppercase ${appMode === 'alim' ? 'bg-red-600/10 text-red-500 border-l-2 border-red-500' : 'text-slate-400 hover:bg-white/5'}`}>Cihaz Alımı</button>
            <button onClick={() => {setAppMode('dis_kanal'); resetAll();}} className={`w-full flex items-center px-4 py-4 rounded-xl text-xs font-bold uppercase ${appMode === 'dis_kanal' ? 'bg-red-600/10 text-red-500 border-l-2 border-red-500' : 'text-slate-400 hover:bg-white/5'}`}>Dış Kanal Listesi</button>
        </div>
        <div className="p-6 border-t border-white/5">
            <div className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10">
                <span className="text-[10px] font-black text-slate-400">ZUMAY KANALI</span>
                <button onClick={handleLogout} className="text-red-500 hover:text-red-400">Çıkış</button>
            </div>
        </div>
      </nav>
      ...
      <main className="flex-1 p-6 md:p-10 h-screen overflow-y-auto">
        {appMode === 'ana_sayfa' ? (
             <div className="flex flex-col items-center justify-center min-h-[70vh] space-y-8">
                ...
                <button onClick={() => {setAppMode('alim'); setStep(1);}} className="bg-red-600 text-white px-8 py-5 rounded-2xl font-black uppercase tracking-widest mt-8">CİHAZ ALIMI YAP</button>
             </div>
        ) : appMode === 'dis_kanal' ? (
            <div className="bg-[#1e1e2d] p-10 rounded-[48px] border border-slate-800">
                <h2 className="text-3xl font-black italic text-red-500 mb-8 uppercase">DIŞ KANAL LİSTESİ</h2>
                <div className="bg-red-700 px-4 py-3 rounded-t-2xl flex font-black text-[10px]">
                    <div className="flex-[3]">ÜRÜN ADI</div><div className="flex-1 text-center">FİYAT</div>
                </div>
                <div className="bg-[#2a2a3d] rounded-b-2xl border border-slate-700">
            ...
            step === 1 ? (
                <div className="space-y-8">
                    <h2 className="text-5xl font-black italic text-center mb-10">CİHAZ <span className="text-red-600">ALIM</span></h2>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {displayBrands.map(brand => {
                            const hasModels = db.some(i => i.brand === brand);
                            return (
                                <div key={brand} onClick={() => { if(hasModels) { setSelectedBrand(brand); setStep(2); } }} className={`bg-[#1e1e2d] p-8 rounded-[40px] border flex flex-col items-center justify-center text-center h-64 ${hasModels ? 'border-slate-700 hover:border-red-500 cursor-pointer' : 'opacity-50 border-slate-800'}`}>
                                    <h2 className="font-black text-xl mb-2 uppercase">{brand}</h2>
                                    <p className="text-[10px] text-slate-500">{hasModels ? 'SEÇ' : 'YAKINDA'}</p>
                                </div>
                            );
                        })}
                    </div>
                ...
                <div className="space-y-8">
                    <button onClick={() => setStep(1)} className="text-slate-400 text-sm font-bold">← Geri Dön</button>
                    <h2 className="text-3xl font-black italic uppercase">{selectedBrand} Modelleri</h2>
                    <input type="text" placeholder="Model Ara..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full p-4 bg-[#1e1e2d] border border-slate-700 rounded-2xl outline-none focus:border-red-500" />
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {Array.from(new Set(db.filter(i => i.brand === selectedBrand).map(i => i.name)))
                        .filter(name => name.toLowerCase().includes(searchQuery.toLowerCase()))
                        .map(name => (
                            <div key={name} onClick={() => {setSelectedModelName(name); setStep(3);}} className="bg-[#1e1e2d] p-6 rounded-[32px] border border-slate-700 hover:border-red-500 cursor-pointer text-center">
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    <div className="flex-1 space-y-6">
                        <button onClick={() => setStep(2)} className="text-slate-400 text-sm font-bold mb-4">← Modellere Dön</button>
                        <div className="bg-[#1e1e2d] p-8 rounded-[40px] border border-slate-700 space-y-6">
                            <h3 className="text-xl font-black italic text-red-500 uppercase">Müşteri Bilgileri</h3>
                            <input placeholder="Ad Soyad" value={customer.name} onChange={(e)=>setCustomer({...customer, name: e.target.value})} className="w-full p-4 bg-[#131722] rounded-xl outline-none focus:border-red-500 border border-slate-700" />
                            <input placeholder="Telefon" value={customer.phone} onChange={(e)=>setCustomer({...customer, phone: e.target.value})} className="w-full p-4 bg-[#131722] rounded-xl outline-none focus:border-red-500 border border-slate-700" />
                            <input placeholder="IMEI" maxLength={15} value={customer.imei} onChange={(e)=>setCustomer({...customer, imei: e.target.value.replace(/\D/g, '')})} className="w-full p-4 bg-[#131722] rounded-xl outline-none focus:border-red-500 border border-slate-700" />
                        </div>
                        <div className="bg-[#1e1e2d] p-8 rounded-[40px] border border-slate-700">
                            ...
                            <div className="flex flex-wrap gap-3">
                                {db.filter(i => i.name === selectedModelName).map(c => (
                                    <button key={c.cap} onClick={() => setSelectedCapacity(c)} className={`px-6 py-3 rounded-xl font-black text-xs ${selectedCapacity?.cap === c.cap ? 'bg-red-600 text-white' : 'bg-[#131722] text-slate-400'}`}>{c.cap}</button>
                                ))}
                            </div>
                        </div>
                        {[
                            ...
                            <div key={q.field} className="bg-[#1e1e2d] p-6 rounded-[32px] border border-slate-700">
                                <p className="text-[10px] font-black mb-4 text-slate-400 uppercase">{q.label}</p>
                                <div className="flex flex-wrap gap-3">
                                    {q.opts.map((opt) => (
                                        <button key={opt} onClick={() => setStatus({...status, [q.field]: opt})} className={`py-3 px-5 rounded-xl text-xs font-black border-2 ${status[q.field] === opt ? 'bg-white text-black border-white' : 'bg-transparent text-slate-400 border-slate-700'}`}>{opt}</button>
                                    ))}
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="lg:w-96 space-y-6 sticky top-10 h-fit">
                        <div className="bg-[#1e1e2d] p-8 rounded-[40px] border border-slate-700 text-center">
                            <p className="text-[10px] text-slate-400 font-bold uppercase mb-2">Nakit Teklifi</p>
                            <p className="text-4xl font-black italic">{allSelected ? `${prices.cash.toLocaleString()} TL` : '---'}</p>
                        </div>
                        <div className="bg-red-600 p-8 rounded-[40px] text-center text-white">
                            <p className="text-[10px] text-red-200 font-bold uppercase mb-2">Takas Teklifi</p>
                            <p className="text-4xl font-black italic">{allSelected ? `${prices.trade.toLocaleString()} TL` : '---'}</p>
                        </div>
                        <div className="bg-[#1a1a24] p-8 rounded-[40px] space-y-4">
                            <button disabled={!canProceed || purchaseType !== null} onClick={() => { setPurchaseType('NAKİT'); handleFinalProcess('NAKİT ALINDI'); }} className={`w-full py-4 rounded-xl font-black uppercase text-[11px] ${!canProceed ? 'opacity-30 bg-slate-800' : 'bg-emerald-600 hover:bg-emerald-500'}`}>✓ Nakit Alındı</button>
                            <button disabled={!canProceed || purchaseType !== null} onClick={() => { setPurchaseType('TAKAS'); handleFinalProcess('TAKAS ALINDI'); }} className={`w-full py-4 rounded-xl font-black uppercase text-[11px] ${!canProceed ? 'opacity-30 bg-slate-800' : 'bg-purple-600 hover:bg-purple-500'}`}>🔄 Takas Alındı</button>
                            <button disabled={!canProceed || purchaseType !== null} onClick={() => { setPurchaseType('ALINMADI'); handleFinalProcess('ALINMADI'); }} className={`w-full py-4 rounded-xl font-black uppercase text-[11px] ${!canProceed ? 'opacity-30 bg-slate-800' : 'bg-slate-800 text-red-400 hover:bg-red-600 hover:text-white'}`}>✕ Alınmadı</button>
                        </div>
                    </div>
                </div>
            )
