...
// WingSM stok sistemiyle birlikte IMEI bazli magaza stok listeleme / detay yonetimi.
// Normal magaza kullanicisi yalniz kendi magazasini yonetir.
// Yonetici mail (admin rolü) yalniz CNET deposunu yonetir.
...
// Super Admin ayrı tutulur; onun tüm mağaza yetkisi devam eder.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Normal mağaza kullanıcısı body'den başka mağazayı zorlayamaz.
    if (
      !user.isSuperAdmin &&
      !managerAccess &&
      requestedBranch &&
...
// Yonetici mail yalnizca CNET deposundaki cihazi duzenleyebilir.
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const ownerBranch = normalizeBranch(existing.current_branch_code);
    const managerAccess = isManagerUser(user);
    ...
    if (oldStatus === 'DETAILS_PENDING' || oldStatus === 'AVAILABLE') {
      // Altı detayın zorunlu olması SADECE CNET deposu için geçerlidir.
      // Diğer mağazalarda eksik detay, cihazı talebe kapatmamalıdır.
      newStatus =
        ownerBranch === 'CNET'
          ? detailsComplete
            ? 'AVAILABLE'
