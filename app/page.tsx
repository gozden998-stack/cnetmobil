...
export default function CnetmobilCmrFinalUltimate() {
  ...
  const loadData = async () => {
    try {
      ...
      if (!res.ok) {
         console.error("API'den veri alınamadı, durum kodu:", res.status);
         setLoading(false);
         return;
      }
      ...
      if (data.cepTablet?.values) setCepTabletData(data.cepTablet.values);
      if (data.yna?.values) setYnaData(data.yna.values);
      if (data.disKanal?.values) setDisKanalData(data.disKanal.values);
      if (data.ikinciEl?.values) setIkinciElData(data.ikinciEl.values);
      if (data.imei?.values) setImeiData(data.imei.values);

      if (data.servis?.values) {
        const loadedServis: any = {};
        data.servis.values.forEach((row: any) => {
           loadedServis[row[0]] = {
      ...
      if (data.config?.values) {
        const m: any = {};
        data.config.values.forEach((row: any) => { 
      ...
      if (data.alimlar?.values) {
        setAlimlar(data.alimlar.values.map((val: any, index: number) => ({ data: val, sheetIndex: index + 2 })));
      }

      if (data.brands?.values) {
        setBrandDb(data.brands.values.map((row: any) => ({ name: row[0], logo: row[1] })));
      }
