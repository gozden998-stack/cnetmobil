...
// Tüm özelliklerin tanımlandığı eksiksiz veri yapısı
interface Satir {
  id: number;
...
const TeknikTakipTablosu = ({ isAdmin = false }: Props) => {
  ...
  const satirKaydet = (yeniVeri: Partial<Satir>) => {
    const tamVeri: Satir = {
      ...
      kayitTarihi: '',
      kaydedildi: true,
      ...yeniVeri, // Gelen verileri al
      id: Date.now(), // En sonda yazarak üstüne yazılmayı (overwrite) önlüyoruz
  ...
  return (
    <div className={`font-sans ${!isAdmin ? 'bg-slate-950 min-h-screen p-4 md:p-8 text-slate-200' : 'p-0 text-slate-200'}`}>
      <div className="max-w-[1400px] mx-auto space-y-8">
        ...
        {/* YÖNETİCİ MODU: İSTATİSTİKLER VE FİLTRELER (Giriş Formu Gizli) */}
        {isAdmin && (
          <div className="animate-in fade-in slide-in-from-top-4 duration-700">
            ...
            <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-[2rem] flex flex-wrap gap-4 items-end shadow-2xl mb-8">
              <div className="flex-1 min-w-[200px]">
                <label className="text-[10px] font-bold text-slate-400 uppercase mb-2 block ml-2">Personel Filtre</label>
              ...
              <div className="flex-1 min-w-[200px]">
                <label className="text-[10px] font-bold text-slate-400 uppercase mb-2 block ml-2">Test Analizi</label>
        ...
        {/* PERSONEL MODU: VERİ GİRİŞİ (Yalnızca Personel Görür) */}
        {!isAdmin && (
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex w-full md:w-fit gap-2 bg-slate-900/80 p-2 rounded-2xl border border-slate-800 shadow-xl mx-auto">
              ...
              <button onClick={() => {
                ...
                if (cihaz) setBulunanCihaz(cihaz); else alert("Giriş kaydı bulunamadı.");
            ...
            {bulunanCihaz ? (
              <div className="bg-indigo-900/10 border border-indigo-500/30 p-8 rounded-[2.5rem] animate-in zoom-in-95">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  ...
                  <input id="cHata" type="text" placeholder="Hata Detayı..." className="bg-slate-950 border border-slate-800 rounded-xl p-4 text-sm outline-none focus:border-indigo-500 text-white" />
              ...
              <tbody className="divide-y divide-slate-800/40">
                {filtrelenmisSatirlar.map((satir) => (
                  <tr key={satir.id} className="group hover:bg-white/[0.02] transition-colors">
                    <td className="p-6 text-sm font-bold text-slate-200 uppercase">{satir.tamirPersoneli}</td>
                    <td className="p-6">
                    ...
                    {isAdmin && (
                      <td className="p-6 text-center">
                        <button onClick={() => setSatirlar(prev => prev.filter(s => s.id !== satir.id))} className="text-slate-600 hover:text-rose-500 p-2 rounded-xl transition-all opacity-0 group-hover:opacity-100">
