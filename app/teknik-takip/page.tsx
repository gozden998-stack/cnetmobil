...
const TeknikTakipTablosu = ({ isAdmin = false }: Props) => {
  ...
  const handleImeiSorgula = () => {
    // Arama yaparken de en azından boş olmamasını veya doğru olmasını kontrol edelim
    if (aramaImei.trim().length === 0) return;
  ...
  const yeniGirisKaydet = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const girilenImei = (formData.get('imei') as string).trim();

    // 15 HANE KONTROLÜ BURADA YAPILIYOR
    if (girilenImei.length < 15) {
      alert("⚠️ HATA: IMEI numarası eksik! Lütfen tam olarak 15 haneli olacak şekilde giriniz.");
      return; // 15 haneden küçükse işlemi durdur, kaydetme!
    }
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 border-b border-slate-800/80 pb-6">
          ...
          {!isAdmin && (
            <div className="flex w-full md:w-auto gap-2 bg-slate-900/50 p-2 rounded-2xl border border-slate-800 shadow-xl focus-within:border-blue-500/50 transition-all">
              {/* ARAMA KUTUSU: MaxLength 15 eklendi */}
              <input 
                type="text" 
                maxLength={15}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
            <div className="bg-slate-900/40 border border-slate-800 rounded-[2rem] p-8 shadow-xl backdrop-blur-md">
              ...
              <form onSubmit={yeniGirisKaydet} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* IMEI KAYIT KUTUSU: MaxLength 15 eklendi */}
                <input 
                  name="imei" 
                  required 
                  type="text" 
                  maxLength={15}
                  placeholder="15 Haneli IMEI Kaydı" 
                  className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm outline-none focus:border-blue-500 font-mono text-white" 
                />
